...
public class UdpMulticastTransport : ITransport
{
    ...
    public UdpMulticastTransport(SyncConfig config, Guid nodeId)
    {
        _config = config;
        _nodeId = nodeId;
    ...
    public Result Start()
    {
        try
        {
            ...
            _udpClient.Client.Bind(new IPEndPoint(IPAddress.Any, _config.ListenPort + 1));
            _udpClient.JoinMulticastGroup(_config.MulticastAddress);
    ...
    public Result Stop()
    {
        try
        {
            ...
            _udpClient?.DropMulticastGroup(_config.MulticastAddress);
            _udpClient?.Close();
            return Result.Success();
        }
        catch (Exception ex)
    ...
    {
        return SendInternal(message, new IPEndPoint(_config.MulticastAddress, _config.MulticastPort + 1));
    }
