...
public class UdpMulticastDiscovery : IPeerDiscovery
{
    ...
    public UdpMulticastDiscovery(SyncConfig config, Guid nodeId)
    {
        _config = config;
        _nodeId = nodeId;
    ...
    private void BroadcastHeartbeat()
    {
        ...
        _ = Task.Run(async () =>
        {
            while (_cts?.IsCancellationRequested == false)
            {
                try
                {
                    await _udpClient!.SendAsync(bytes, bytes.Length, new IPEndPoint(_config.MulticastAddress, _config.MulticastPort));
